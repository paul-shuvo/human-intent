...
os.chdir(join(dir_path, 'AlphaPose'))
print(os.getcwd())
...
# Following value are added to the default argument
# --cfg configs/coco/resnet/256x192_res50_lr1e-3_1x.yaml --checkpoint pretrained_models/fast_res50_256x192.pth
# changed those arguments to required=False
"""----------------------------- Demo options -----------------------------"""
parser = argparse.ArgumentParser(description='AlphaPose Single-Image Demo')
parser.add_argument('--cfg', default='configs/coco/resnet/256x192_res50_lr1e-3_1x.yaml',
                    type=str, required=False, help='experiment configure file name')
parser.add_argument('--checkpoint', default='pretrained_models/fast_res50_256x192.pth',
                    type=str, required=False, help='checkpoint file name')
parser.add_argument('--detector', dest='detector',
                    help='detector name', default="yolo")
parser.add_argument('--image', dest='inputimg',
                    help='image-name', default="examples/demo/1.jpg")
...
class DetectionLoader():
    ...
    def image_preprocess(self, im_name, image):
        ...
        method_list = [func for func in dir(self.detector) if callable(getattr(self.detector, func))]
...
class ForearmPosePublish():
    def __init__(self, args, cfg):
        self.alphapose = SingleImageAlphaPose(args, cfg)
        self.im_name = 'kinect image'
        self.frame_rate = 2
        self.prev = 0
        self.msg = Floats()
    ...
    def callback(self, kinect_image):
        ...
        if cv2.waitKey(30) & 0xFF == ord('q'):
            rospy.signal_shutdown('Received signal for shutdown')



if __name__ == "__main__":
    ForearmPosePublish(args, cfg)
# rospy.init_node("rospy_rate_test")
# pub = rospy.Publisher("/chatter", Floats, queue_size=10)
# msg = Floats()
# msg.header.stamp = rospy.Time.now()
# msg.data = np.zeros((8,))
# alphapose = SingleImageAlphaPose(args, cfg)
...
#     # if time_elapsed > 1. / self.frame_rate:
#     #     self.prev = time.time()
#     global alphapose, msg
#     pose = alphapose.process('kinect image', image)
#     if pose is not None and pose['result'][0]['keypoints'].shape[0] is 17:
#         # msg = Floats()
#         msg.header.stamp = rospy.Time.now()
#         msg.data = pose['result'][0]['keypoints'].cpu().detach().numpy()[7:11, :].flatten('C')
#         # print(msg.data.shape)
