...
parser.add_argument(
    "--image", dest="inputimg", help="image-name", default="examples/demo/1.jpg"
)
parser.add_argument(
...
parser.add_argument(
    "--showbox", default=False, action="store_true", help="visualize human bbox"
)
parser.add_argument(
    "--profile",
    default=False,
...
class DetectionLoader:
    ...
    def image_preprocess(self, im_name, image):
        ...
        method_list = [
            func
            for func in dir(self.detector)
            if callable(getattr(self.detector, func))
        ]
    ...
    def update(self):
        ...
        if boxes is None or len(boxes) == 0:
            ...
            boxes, scores, ids, preds_img, preds_scores, pick_ids = pose_nms(
                boxes, scores, ids, preds_img, preds_scores, self.opt.min_box_area
            )
...
class ForearmPosePublish:
    ...
    def callback(self, kinect_image):
        image = np.frombuffer(kinect_image.data, dtype=np.uint8).reshape(
            kinect_image.height, kinect_image.width, -1
        )
