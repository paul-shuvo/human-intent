...
class CanvasInteraction:
    def __init__(self, config, topics):
        ...
        self.compute_canvas_once = self.config["canvas_interaction"][
            "compute_canvas_once"
        ]
        ...
        subscribers = [
            message_filters.Subscriber(topic["topic_name"], topic["data_type"])
        ...
        ts = message_filters.ApproximateTimeSynchronizer(
            subscribers, 10, 1, allow_headerless=True
        )  # Changed code
    ...
    def update_points_arm_3d(self, forearm_pose, pointcloud, arm="left"):
        ...
        for pt_count, dt in enumerate(
            pc2.read_points(
                ...
                field_names={"x", "y", "z"},
                skip_nans=True,
                uvs=arb_arm_points.astype(int).tolist()[5:10],
            )
        ):
            # print(type(dt))
            if pt_count == 2:
    ...
    def callback(self, forearm_pose, pointcloud):
        ...
        try:
            self.left_arm_line_3D = Line3D(
                Point3D(*self.arm_points_3D[0, :]), Point3D(*self.arm_points_3D[1, :])
            )
            self.canvas_plane = Plane(
                Point3D(*self.canvas_pts_3D[0, :]),
                Point3D(*self.canvas_pts_3D[1, :]),
                Point3D(*self.canvas_pts_3D[2, :]),
            )
        except ValueError as error:
            rospy.loginfo(error)

        self.canvas_plane_point = self.canvas_plane.intersection(self.left_arm_line_3D)[
            0
        ].evalf()
    ...
    def get_3d_points_on_plane(self, bounding_box, pointcloud, n_points=10):
        ...
        points = (
            np.vstack(
                (
                    points_on_triangle(bounding_box[:3], n_points),
                    points_on_triangle(bounding_box[1:], n_points),
                )
            )
            .astype(int)
            .tolist()
