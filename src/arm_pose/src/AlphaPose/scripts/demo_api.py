...
import time
print(os.getcwd())
# os.chdir(os.path.join(os.getcwd(), 'src/arm_pose/src/Alphapose'))
...
def example():
    ...
    while (True):
        ...
        if time_elapsed > 1. / frame_rate:
            ...
            # img = demo.vis(image, pose)  # visulize the pose result
            # cv2.imwrite(os.path.join(outputpath, 'vis', os.path.basename(im_name)), img)
            if pose is not None:
                for i, point in enumerate(pose['result'][0]['keypoints']):
                    if i in [7, 8, 9, 10]:
                    # print(point)
                        cv2.circle(image, tuple(point.type(torch.ShortTensor).tolist()), radius=5, color=(0,255,0), thickness=1, lineType=8, shift=0)
            # if you want to vis the img:
            # print('here')
            cv2.imshow("AlphaPose Demo", image)
            cv2.waitKey(30)
            # time.sleep(0.5)
    ...
    # image = cv2.cvtColor(cv2.imread(im_name), cv2.COLOR_BGR2RGB) #.swapaxes(0, 2).swapaxes(1,2)
    # cv2.imshow("AlphaPose Demo", image)
    # cv2.waitKey(3000)
    # pose = demo.process(im_name, image)
    # # img = demo.getImg()     # or you can just use: img = cv2.imread(image)
